...
function replaceClassInComponent(
  ...
  // Strategy: find the oldClass within string literals in the source.
  // If variantContext is provided, narrow to the section near that variant name.

  if (variantContext) {
    // Find the variant context in the source (e.g. `default:` or `destructive:`)
    const variantIndex = source.indexOf(`${variantContext}:`);
    if (variantIndex === -1) {
      // Try with quotes: "default":
  ...
  // Find the old class as a whole word within a string literal
  // Match: "...oldClass..." or '...oldClass...' or `...oldClass...`
  ...
  if (variantContext) {
    // Only replace within the variant's string
    // Find the variant section and replace only there
    ...
    // Also try quoted variant key: "default":
    if (!replaced) {
  ...
  if (!replaced) {
    // Fallback: replace in any string literal
